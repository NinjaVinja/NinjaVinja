<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0d0d,50:8B0000,100:0d0d0d&height=230&section=header&text=NinjaVinja&fontSize=65&fontColor=ffffff&animation=fadeIn&fontAlignY=32&desc=見えない里より、コードを届ける&descAlignY=52&descSize=16" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=800&color=E63946&center=true&vCenter=true&width=650&lines=Genin+of+the+Code+Village+%F0%9F%8D%83;BSCS+Student+%40+UCP+Lahore;Training+in+Python+%2B+AI%2FML+Jutsu;Forging+Harfnama+%26+Vein2Vein;Akatsuki+at+heart+%E2%98%81%EF%B8%8F" alt="Typing SVG" />

<br>

![Rank](https://img.shields.io/static/v1?label=RANK&message=GENIN%20%E2%86%92%20CHUNIN&color=8B0000&style=for-the-badge&labelColor=0d0d0d)
![Village](https://img.shields.io/static/v1?label=VILLAGE&message=UCP%20LAHORE&color=8B0000&style=for-the-badge&labelColor=0d0d0d)
![Chakra](https://img.shields.io/static/v1?label=CHAKRA%20NATURE&message=PYTHON%20%2B%20AI%2FML&color=8B0000&style=for-the-badge&labelColor=0d0d0d)

</div>

<div align="center">☁️ ▬▬▬▬▬▬▬▬▬▬▬▬ ⛩️ ▬▬▬▬▬▬▬▬▬▬▬▬ ☁️</div>

## 🈲 Scroll of Self

I'm **Muhammad Taha Ahmad** — codename `NinjaVinja`. 7th-semester BSCS student at the **University of Central Punjab**, currently ranked Genin in the ways of Python and AI/ML, training hard for a promotion.

```python
class NinjaVinja(Shinobi):
    rank        = "Genin → Chunin (in progress)"
    village     = "University of Central Punjab, Lahore"
    chakra_type = ["Python", "AI/ML", "FastAPI", "TensorFlow/PyTorch"]
    jutsu_in_progress = ["Harfnama", "Vein2Vein"]
    sensei      = "Andrew Ng (ML Specialization)"
    allegiance  = "Akatsuki (unofficially)"

    def status(self):
        return "🔴 On a solo training arc — sharpening AI/ML jutsu"
```

- 🍥 7th-semester BSCS, CGPA **3.70/4.00** — steady grinding, no shortcuts
- 🥷 Training arc: **NumPy/Pandas → Scikit-learn → TensorFlow/PyTorch → FastAPI**
- 📜 Running daily drills through **100 Days of Code**
- ☁️ Off the clock: Akatsuki lore, gaming, and rice-ing my desktop setup
- 📬 Summon me: **taha.ahmad4700@gmail.com**

<div align="center">☁️ ▬▬▬▬▬▬▬▬▬▬▬▬ ⛩️ ▬▬▬▬▬▬▬▬▬▬▬▬ ☁️</div>

## ⛩️ Forbidden Scrolls (Featured Projects)

<table>
<tr>
<td width="50%" valign="top">

**📝 Harfnama** — *Fūinjutsu: Nastaliq Style*

Document-to-font converter with Urdu/Nastaliq support, multiple export formats, rich text formatting. Deployed via GitHub Pages/Netlify.

`HTML` `CSS` `JavaScript`

</td>
<td width="50%" valign="top">

**🩸 Vein2Vein** — *Jutsu: Blood Bond Technique*

Flask-based donor-recipient matching platform, admin approval workflow, live map tracking, hardened auth (bcrypt + sessions).

`Flask` `Python` `Leaflet.js`

</td>
</tr>
</table>

<div align="center">☁️ ▬▬▬▬▬▬▬▬▬▬▬▬ ⛩️ ▬▬▬▬▬▬▬▬▬▬▬▬ ☁️</div>

## ⚔️ Jutsu Arsenal

<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="42" alt="python" title="Python"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" height="42" alt="cpp" title="C++"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" height="42" alt="pandas" title="Pandas"/>
  <img src="https://img.shields.io/badge/Turtle-2E7D32?style=for-the-badge&logoColor=white" height="42" alt="turtle" title="Turtle Graphics"/>
  <img src="https://img.shields.io/badge/Tkinter-8B0000?style=for-the-badge&logoColor=white" height="42" alt="tkinter" title="Tkinter"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="42" alt="git" title="Git"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" height="42" alt="tensorflow" title="TensorFlow"/>
</div>

<div align="center">☁️ ▬▬▬▬▬▬▬▬▬▬▬▬ ⛩️ ▬▬▬▬▬▬▬▬▬▬▬▬ ☁️</div>

## 📊 Mission Log (Stats)

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=NinjaVinja&hide_title=false&hide_rank=false&show_icons=true&include_all_commits=true&theme=github_dark&hide_border=true&bg_color=0d0d0d&title_color=E63946&icon_color=E63946&text_color=ffffff" height="165" alt="stats graph" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=NinjaVinja&layout=compact&theme=github_dark&hide_border=true&langs_count=6&bg_color=0d0d0d&title_color=E63946&text_color=ffffff" height="165" alt="languages graph" />
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=NinjaVinja&theme=github-dark-blue&hide_border=true&background=0d0d0d&ring=E63946&fire=E63946&currStreakLabel=E63946" alt="streak stats" />
</div>

<div align="center">☁️ ▬▬▬▬▬▬▬▬▬▬▬▬ ⛩️ ▬▬▬▬▬▬▬▬▬▬▬▬ ☁️</div>

## 🕸️ Send a Hawk (Connect)

<div align="center">
  <a href="mailto:taha.ahmad4700@gmail.com">
    <img src="https://img.shields.io/static/v1?message=Gmail&logo=gmail&label=&color=8B0000&logoColor=white&labelColor=0d0d0d&style=for-the-badge" height="38" />
  </a>
  <a href="https://github.com/NinjaVinja">
    <img src="https://img.shields.io/static/v1?message=GitHub&logo=github&label=&color=8B0000&logoColor=white&labelColor=0d0d0d&style=for-the-badge" height="38" />
  </a>
  <a href="https://www.linkedin.com/in/muhammad-taha-ahmad-391679262/">
    <img src="https://img.shields.io/static/v1?message=LinkedIn&logo=linkedin&label=&color=8B0000&logoColor=white&labelColor=0d0d0d&style=for-the-badge" height="38" />
  </a>
</div>

<br>

<div align="center">
  <i>"A true shinobi doesn't run from a stack trace — they walk the path of pain until it compiles." ☁️</i>
</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0d0d,50:8B0000,100:0d0d0d&height=100&section=footer" width="100%"/>

</div>
